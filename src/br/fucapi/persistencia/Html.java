package br.fucapi.persistencia;

public class Html {
	
	private static String abreHtml = "<!DOCTYPE html><html>";
	private static String abreHead = "<head>";
	private static String abreTitle = "<title>";
	private static String abreBody = "<body>";
	private static String abreH1 = "<h1>";
	private static String abreH2 = "<h2>";
	private static String abreH3 = "<h3>";
	private static String abreH4 = "<h4>";
	private static String abreH5 = "<h5>";
	private static String abreH6 = "<h6>";
	private static String abreHr = "<hr>";
	private static String abreP = "<p>";
	private static String abreBr = "<br>";
	private static String abreB = "<b>";
	private static String abreEm = "<em>";
	private static String abreI = "<i>";
	private static String abreSmall = "<small>";
	private static String abreStrong = "<strong>";
	private static String abreSub = "<sub>";
	private static String abreSup = "<sup>";
	private static String abreIns = "<ins>";
	private static String abreDel = "<del>";
	private static String abreMark = "<mark>";
	private static String abreCode = "<code>";
	private static String abreKbd = "<kbd>";
	private static String abreSamp = "<samp>";
	private static String abreVar = "<var>";
	private static String abrePre = "<pre>";
	private static String abreAbbr = "<abbr>";
	private static String abreAddress = "<address>";
	private static String abreBdo = "<bdo>";
	private static String abreBlockquote = "<blockquote>";
	private static String abreQ = "<q>";
	private static String abreDfn = "<dfn>";
	private static String abreCite = "<cite>";
	private static String abreDiv = "<div>";
	private static String abreSpan = "<span>";
	private static String abreUl = "<ul>";
	private static String abreOl = "<ol>";
	private static String abreLi = "<li>";
	private static String abreDl = "<dl>";
	private static String abreDt = "<dt>";
	private static String abreDd = "<dd>";
	
	private static String fechaHtml = "</html>";
	private static String fechaHead = "</head>";
	private static String fechaTitle = "</title>";
	private static String fechaBody = "</body>";
	private static String fechaH1 = "</h1>";
	private static String fechaH2 = "</h2>";
	private static String fechaH3 = "</h3>";
	private static String fechaH4 = "</h4>";
	private static String fechaH5 = "</h5>";
	private static String fechaH6 = "</h6>";
	private static String fechaHr = "</hr>";
	private static String fechaP = "</p>";
	private static String fechaBr = "</br>";
	private static String fechaB = "</b>";
	private static String fechaEm = "</em>";
	private static String fechaI = "</i>";
	private static String fechaSmall = "</small>";
	private static String fechaStrong = "</strong>";
	private static String fechaSub = "</sub>";
	private static String fechaSup = "</sup>";
	private static String fechaIns = "</ins>";
	private static String fechaDel = "</del>";
	private static String fechaMark = "</mark>";
	private static String fechaCode = "</code>";
	private static String fechaKbd = "</kbd>";
	private static String fechaSamp = "</samp>";
	private static String fechaVar = "</var>";
	private static String fechaPre = "</pre>";
	private static String fechaAbbr = "</abbr>";
	private static String fechaAddress = "</address>";
	private static String fechaBdo = "</bdo>";
	private static String fechaBlockquote = "</blockquote>";
	private static String fechaQ = "</q>";
	private static String fechaDfn = "</dfn>";
	private static String fechaCite = "</cite>";
	private static String fechaDiv = "</div>";
	private static String fechaSpan = "</span>";
	private static String fechaUl = "</ul>";
	private static String fechaOl = "</ol>";
	private static String fechaLi = "</li>";
	private static String fechaDl = "</dl>";
	private static String fechaDt = "</dt>";
	private static String fechaDd = "</dd>";
	
	public String getAbreHtml() {
		return abreHtml;
	}
	public String getAbreHead() {
		return abreHead;
	}
	public String getAbreTitle() {
		return abreTitle;
	}
	public String getAbreBody() {
		return abreBody;
	}
	public String getAbreH1() {
		return abreH1;
	}
	public String getAbreH2() {
		return abreH2;
	}
	public String getAbreH3() {
		return abreH3;
	}
	public String getAbreH4() {
		return abreH4;
	}
	public String getAbreH5() {
		return abreH5;
	}
	public String getAbreH6() {
		return abreH6;
	}
	public String getAbreHr() {
		return abreHr;
	}
	public String getAbreP() {
		return abreP;
	}
	public String getAbreBr() {
		return abreBr;
	}
	public String getAbreB() {
		return abreB;
	}
	public String getAbreEm() {
		return abreEm;
	}
	public String getAbreI() {
		return abreI;
	}
	public String getAbreSmall() {
		return abreSmall;
	}
	public String getAbreStrong() {
		return abreStrong;
	}
	public String getAbreSub() {
		return abreSub;
	}
	public String getAbreSup() {
		return abreSup;
	}
	public String getAbreIns() {
		return abreIns;
	}
	public String getAbreDel() {
		return abreDel;
	}
	public String getAbreMark() {
		return abreMark;
	}
	public String getAbreCode() {
		return abreCode;
	}
	public String getAbreKbd() {
		return abreKbd;
	}
	public String getAbreSamp() {
		return abreSamp;
	}
	public String getAbreVar() {
		return abreVar;
	}
	public String getAbrePre() {
		return abrePre;
	}
	public String getAbreAbbr() {
		return abreAbbr;
	}
	public String getAbreAddress() {
		return abreAddress;
	}
	public String getAbreBdo() {
		return abreBdo;
	}
	public String getAbreBlockquote() {
		return abreBlockquote;
	}
	public String getAbreQ() {
		return abreQ;
	}
	public String getAbreDfn() {
		return abreDfn;
	}
	public String getAbreCite() {
		return abreCite;
	}
	public String getAbreDiv() {
		return abreDiv;
	}
	public String getAbreSpan() {
		return abreSpan;
	}
	public String getFechaHtml() {
		return fechaHtml;
	}
	public String getFechaHead() {
		return fechaHead;
	}
	public String getFechaTitle() {
		return fechaTitle;
	}
	public String getFechaBody() {
		return fechaBody;
	}
	public String getFechaH1() {
		return fechaH1;
	}
	public String getFechaH2() {
		return fechaH2;
	}
	public String getFechaH3() {
		return fechaH3;
	}
	public String getFechaH4() {
		return fechaH4;
	}
	public String getFechaH5() {
		return fechaH5;
	}
	public String getFechaH6() {
		return fechaH6;
	}
	public String getFechaHr() {
		return fechaHr;
	}
	public String getFechaP() {
		return fechaP;
	}
	public String getFechaBr() {
		return fechaBr;
	}
	public String getFechaB() {
		return fechaB;
	}
	public String getFechaEm() {
		return fechaEm;
	}
	public String getFechaI() {
		return fechaI;
	}
	public String getFechaSmall() {
		return fechaSmall;
	}
	public String getFechaStrong() {
		return fechaStrong;
	}
	public String getFechaSub() {
		return fechaSub;
	}
	public String getFechaSup() {
		return fechaSup;
	}
	public String getFechaIns() {
		return fechaIns;
	}
	public String getFechaDel() {
		return fechaDel;
	}
	public String getFechaMark() {
		return fechaMark;
	}
	public String getFechaCode() {
		return fechaCode;
	}
	public String getFechaKbd() {
		return fechaKbd;
	}
	public String getFechaSamp() {
		return fechaSamp;
	}
	public String getFechaVar() {
		return fechaVar;
	}
	public String getFechaPre() {
		return fechaPre;
	}
	public String getFechaAbbr() {
		return fechaAbbr;
	}
	public String getFechaAddress() {
		return fechaAddress;
	}
	public String getFechaBdo() {
		return fechaBdo;
	}
	public String getFechaBlockquote() {
		return fechaBlockquote;
	}
	public String getFechaQ() {
		return fechaQ;
	}
	public String getFechaDfn() {
		return fechaDfn;
	}
	public String getFechaCite() {
		return fechaCite;
	}
	public String getFechaDiv() {
		return fechaDiv;
	}
	public String getFechaSpan() {
		return fechaSpan;
	}
	public static String getAbreUl() {
		return abreUl;
	}
	public static String getAbreOl() {
		return abreOl;
	}
	public static String getAbreLi() {
		return abreLi;
	}
	public static String getAbreDl() {
		return abreDl;
	}
	public static String getAbreDt() {
		return abreDt;
	}
	public static String getAbreDd() {
		return abreDd;
	}
	public static String getFechaUl() {
		return fechaUl;
	}
	public static String getFechaOl() {
		return fechaOl;
	}
	public static String getFechaLi() {
		return fechaLi;
	}
	public static String getFechaDl() {
		return fechaDl;
	}
	public static String getFechaDt() {
		return fechaDt;
	}
	public static String getFechaDd() {
		return fechaDd;
	}
	
	
}
